...
export default function App() {
  ...
  const ListView = () => (
    <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: .18 }}>
      ...
      <section style={{ background: C.shellBg, padding: `${S.s11} ${S.s6}` }}>
        <div style={{ maxWidth: 1056, margin: '0 auto' }}>
          ...
          <div style={{ maxWidth: 672 }}>
            ...
            <h1 style={{ ...T.heading07, color: C.shellText, marginBottom: S.s5, fontFamily: "'IBM Plex Sans', sans-serif" }}>
              방문 전<br />QR 길잡이
            </h1>
          ...
          {/* Stat row */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 0, marginTop: S.s10, borderTop: `1px solid #393939` }}>
            {[
              { num: '4', label: '민원 카테고리' },
              ...
              <div key={i} style={{ padding: `${S.s7} ${S.s6} ${S.s7} 0`, borderRight: i < 3 ? `1px solid #393939` : 'none' }}>
                <div style={{ ...T.heading06, color: C.shellText, fontFamily: "'IBM Plex Sans', sans-serif" }}>{s.num}</div>
      ...
      {/* ── CARDS SECTION ── */}
      <section style={{ background: C.layer01, padding: `${S.s10} ${S.s6}` }}>
        <div style={{ maxWidth: 1056, margin: '0 auto' }}>
          ...
          {/* Card grid */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1px', background: C.borderSubtle01 }}>
      ...
      <section style={{ background: C.background, padding: `${S.s10} ${S.s6}` }}>
        <div style={{ maxWidth: 1056, margin: '0 auto' }}>
      ...
      {/* ── GLOSSARY (Gray 100) ── */}
      <section style={{ background: C.shellBg, padding: `${S.s10} ${S.s6}` }}>
        <div style={{ maxWidth: 1056, margin: '0 auto' }}>
      ...
      {/* ── DISCLAIMER ── */}
      <section style={{ background: C.background, padding: `${S.s8} ${S.s6}` }}>
        <div style={{ maxWidth: 1056, margin: '0 auto' }}>
      ...
      {/* ── FOOTER ── */}
      <footer style={{ background: C.shellBg, padding: `${S.s9} ${S.s6} ${S.s7}`, borderTop: `1px solid #393939` }}>
        <div style={{ maxWidth: 1056, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: S.s9, marginBottom: S.s9, paddingBottom: S.s7, borderBottom: '1px solid #393939' }}>
            {[
              ...
              <div key={i}>
                <div style={{ ...T.heading01, color: C.shellText, marginBottom: S.s5, fontFamily: "'IBM Plex Sans', 'Noto Sans KR', sans-serif" }}>{col.head}</div>
                <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {col.links.map((l, j) => (
                    <li key={j}><span style={{ ...T.body01, color: '#c6c6c6', fontFamily: "'IBM Plex Sans', 'Noto Sans KR', sans-serif" }}>{l}</span></li>
                  ))}
                </ul>
              </div>
            ))}
          </div>
          <div style={{ ...T.label01, color: '#6f6f6f', fontFamily: "'IBM Plex Sans', sans-serif" }}>
            © 2026 방문 전 안심 길잡이 (비공식 도우미). All rights reserved. &nbsp;|&nbsp; 개인정보 미수집 · 서버 미사용
          </div>
        </div>
      </footer>
    </motion.div>
  ...
  const DetailView = ({ card }: { card: ComplaintCard }) => {
    ...
    return (
      <motion.div key="detail" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={{ duration: .18 }}>

        {/* Breadcrumb nav */}
        <div style={{ background: C.layer01, padding: `${S.s4} ${S.s6}`, borderBottom: `1px solid ${C.borderSubtle01}`, position: 'sticky', top: 48, zIndex: 40 }}>
          <div style={{ maxWidth: 1056, margin: '0 auto', display: 'flex', alignItems: 'center', gap: S.s4 }}>
            <button onClick={backToList}
              style={{ display: 'flex', alignItems: 'center', gap: 6, ...T.bodyCompact01, color: C.interactive, cursor: 'pointer', background: 'none', border: 'none', fontFamily: "'IBM Plex Sans', 'Noto Sans KR', sans-serif" }}>
              <ArrowLeft size={16} /> 민원 목록
            </button>
            <span style={{ color: C.borderSubtle01 }}>/</span>
            <span style={{ ...T.bodyCompact01, color: C.textSecondary, fontFamily: "'IBM Plex Sans', 'Noto Sans KR', sans-serif" }}>{card.category}</span>
          </div>
        </div>

        {/* Page header */}
        <section style={{ background: C.shellBg, padding: `${S.s9} ${S.s6}` }}>
          <div style={{ maxWidth: 1056, margin: '0 auto' }}>
            <div style={{ display: 'flex', gap: S.s3, marginBottom: S.s5, flexWrap: 'wrap', alignItems: 'center' }}>
              <CarbonTag label={card.category} bg={tag.bg} text={tag.text} />
              <span style={{ ...T.label01, color: '#6f6f6f', fontFamily: "'IBM Plex Sans', 'Noto Sans KR', sans-serif" }}>{card.target}</span>
            </div>
            <h1 style={{ ...T.heading06, color: C.shellText, marginBottom: S.s5, fontFamily: "'IBM Plex Sans', 'Noto Sans KR', sans-serif", maxWidth: 672 }}>
              {card.title}
            </h1>
            <p style={{ ...T.body02, color: '#c6c6c6', marginBottom: S.s7, maxWidth: 560, fontFamily: "'IBM Plex Sans', 'Noto Sans KR', sans-serif" }}>
              {card.tagLine}
            </p>
            <div style={{ display: 'flex', gap: 0, flexWrap: 'wrap' }}>
              <CarbonBtn kind="primary" href={getOnlineLink(card)} icon={<ExternalLink size={16} />}>{card.onlineUrlLabel}</CarbonBtn>
              <CarbonBtn kind="secondary" href={getFormLink()} icon={<FileDown size={16} />}>서식 다운</CarbonBtn>
            </div>
          </div>
        </section>

        {/* Body */}
        <div style={{ maxWidth: 1056, margin: '0 auto', padding: `0 ${S.s6} ${S.s11}` }}>

          {/* Two-col layout on wide */}
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,2fr) minmax(0,1fr)', gap: S.s9, alignItems: 'start' }}>

            {/* Left column */}
            <div>
              {/* What is this */}
              <div style={{ padding: `${S.s8} 0`, borderBottom: `1px solid ${C.borderSubtle01}` }}>
                <div style={{ ...T.label01, color: C.textSecondary, textTransform: 'uppercase', letterSpacing: '.32px', marginBottom: S.s5, fontFamily: "'IBM Plex Sans', sans-serif" }}>이 민원은 무엇인가요?</div>
                <p style={{ ...T.body01, color: C.textPrimary, lineHeight: '20px', fontFamily: "'IBM Plex Sans', 'Noto Sans KR', sans-serif" }}>{card.whatIsThis}</p>
              </div>

              {/* Who applies */}
              <div style={{ padding: `${S.s8} 0`, borderBottom: `1px solid ${C.borderSubtle01}` }}>
                <div style={{ ...T.label01, color: C.textSecondary, textTransform: 'uppercase', letterSpacing: '.32px', marginBottom: S.s5, fontFamily: "'IBM Plex Sans', sans-serif" }}>누가 신청하나요?</div>
                <p style={{ ...T.body01, color: C.textPrimary, lineHeight: '20px', fontFamily: "'IBM Plex Sans', 'Noto Sans KR', sans-serif" }}>{card.whoApplies}</p>
              </div>

              {/* Cases */}
              <div style={{ padding: `${S.s8} 0`, borderBottom: `1px solid ${C.borderSubtle01}` }}>
                <div style={{ ...T.label01, color: C.textSecondary, textTransform: 'uppercase', letterSpacing: '.32px', marginBottom: S.s5, fontFamily: "'IBM Plex Sans', sans-serif" }}>대표 적용 사례</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: S.s5 }}>
                  {card.cases.map((c, i) => (
                    <div key={i} style={{ display: 'flex', gap: S.s5, alignItems: 'flex-start' }}>
                      <span style={{ ...T.label01, color: C.textOnColor, background: C.interactive, padding: '3px 8px', whiteSpace: 'nowrap', flexShrink: 0, fontFamily: "'IBM Plex Sans', sans-serif" }}>
                        사례 {i + 1}
                      </span>
                      <p style={{ ...T.body01, color: C.textPrimary, lineHeight: '20px', fontFamily: "'IBM Plex Sans', 'Noto Sans KR', sans-serif" }}>{c}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Procedures */}
              <div style={{ padding: `${S.s8} 0`, borderBottom: `1px solid ${C.borderSubtle01}` }}>
                <div style={{ ...T.label01, color: C.textSecondary, textTransform: 'uppercase', letterSpacing: '.32px', marginBottom: S.s7, fontFamily: "'IBM Plex Sans', sans-serif" }}>처리 절차</div>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  {card.procedures.map((proc, i) => (
                    <div key={i} style={{ display: 'flex', gap: S.s5, paddingBottom: S.s6 }}>
                      <div style={{
                        width: 24, height: 24, flexShrink: 0,
                        background: i < 2 ? C.supportSuccess : C.layer01,
                        border: `2px solid ${i < 2 ? C.supportSuccess : C.borderStrong01}`,
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        ...T.label01, color: i < 2 ? C.textOnColor : C.textSecondary,
                        fontFamily: "'IBM Plex Sans', sans-serif",
                      }}>
                        {i < 2 ? <Check size={14} /> : i + 1}
                      </div>
                      <div>
                        <span style={{ ...T.label01, color: i < 2 ? C.supportSuccess : i === 2 ? C.interactive : C.textHelper, fontFamily: "'IBM Plex Sans', sans-serif" }}>
                          {i + 1}단계{i < 2 ? ' ✓' : i === 2 ? ' · 현재' : ''}
                        </span>
                        <p style={{ ...T.body01, color: i < 2 ? C.textSecondary : C.textPrimary, lineHeight: '20px', marginTop: 2, fontFamily: "'IBM Plex Sans', 'Noto Sans KR', sans-serif" }}>{proc}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {/* Online guidance */}
              <div style={{ padding: `${S.s8} 0`, borderBottom: `1px solid ${C.borderSubtle01}` }}>
                <div style={{ ...T.label01, color: C.textSecondary, textTransform: 'uppercase', letterSpacing: '.32px', marginBottom: S.s5, fontFamily: "'IBM Plex Sans', sans-serif" }}>비대면 온라인 신청</div>
                <div style={{ background: C.layer01, padding: S.s6, borderLeft: `4px solid ${C.interactive}` }}>
                  <p style={{ ...T.body01, color: C.textPrimary, lineHeight: '20px', fontFamily: "'IBM Plex Sans', 'Noto Sans KR', sans-serif" }}>{card.onlineGuidance}</p>
                </div>
              </div>

              {/* Caution */}
              <div style={{ padding: `${S.s8} 0` }}>
                <div style={{ ...T.label01, color: C.supportError, textTransform: 'uppercase', letterSpacing: '.32px', marginBottom: S.s5, fontFamily: "'IBM Plex Sans', sans-serif" }}>주의 사항</div>
                <div style={{ background: '#fff1f1', borderLeft: `4px solid ${C.supportError}`, padding: S.s6 }}>
                  <p style={{ ...T.body01, color: C.textPrimary, lineHeight: '20px', fontFamily: "'IBM Plex Sans', 'Noto Sans KR', sans-serif" }}>{card.caution}</p>
                </div>
              </div>
            </div>
            ...
            <div style={{ position: 'sticky', top: 104 }}>
              ...
              {/* Pre-checklist */}
              <div style={{ background: C.background, border: `1px solid ${C.borderSubtle01}`, marginBottom: S.s5 }}>
                <div style={{ padding: S.s5, borderBottom: `1px solid ${C.borderSubtle01}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ ...T.heading01, color: C.textPrimary, fontFamily: "'IBM Plex Sans', 'Noto Sans KR', sans-serif" }}>준비 자가체크</div>
                  <span style={{ ...T.label01, color: C.interactive, fontFamily: "'IBM Plex Sans', sans-serif" }}>{preSt.done}/{preSt.total}</span>
                </div>
                {card.preChecklist.map((item, i) => (
                  <CheckRow key={i} label={item} checked={!!preChecklist[card.id]?.[i]} onChange={() => togglePre(card.id, i)} />
                ))}
                <ProgressBar pct={preSt.pct} />
              </div>

              {/* Required docs */}
              <div style={{ background: C.background, border: `1px solid ${C.borderSubtle01}`, marginBottom: S.s5 }}>
                <div style={{ padding: S.s5, borderBottom: `1px solid ${C.borderSubtle01}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ ...T.heading01, color: C.supportError, fontFamily: "'IBM Plex Sans', 'Noto Sans KR', sans-serif" }}>★ 필수 지참 서류</div>
                  <span style={{ ...T.label01, color: C.supportSuccess, fontFamily: "'IBM Plex Sans', sans-serif" }}>{docSt.done}/{docSt.total}</span>
                </div>
                {card.requiredDocs.map((doc, i) => (
                  <CheckRow key={i} label={doc} checked={!!checklist[card.id]?.[i]} onChange={() => toggleDoc(card.id, i)} strikethrough />
                ))}
                <ProgressBar pct={docSt.pct} color={C.supportSuccess} />
              </div>

              {/* Helpful docs */}
              {card.helpfulDocs.length > 0 && (
                <div style={{ background: C.layer01, border: `1px solid ${C.borderSubtle01}`, padding: S.s5, marginBottom: S.s5 }}>
                  <div style={{ ...T.heading01, color: C.textSecondary, marginBottom: S.s4, fontFamily: "'IBM Plex Sans', 'Noto Sans KR', sans-serif" }}>🔹 가져오면 빠른 서류</div>
                  <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: S.s4 }}>
                    {card.helpfulDocs.map((doc, i) => (
                      <li key={i} style={{ display: 'flex', gap: S.s3, ...T.body01, color: C.textSecondary, fontFamily: "'IBM Plex Sans', 'Noto Sans KR', sans-serif" }}>
                        <span style={{ color: C.interactive }}>→</span> {doc}
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                <CarbonBtn kind="primary" size="lg" href={getOnlineLink(card)} icon={<ExternalLink size={16} />}>
                  {card.onlineUrlLabel}
                </CarbonBtn>
                <CarbonBtn kind="secondary" size="lg" href="tel:1588-0075" icon={<Phone size={16} />}>
                  1588-0075
                </CarbonBtn>
                <CarbonBtn kind="secondary" size="lg" href="https://www.comwel.or.kr/comwel/info/data/papr/papr_lst.jsp" icon={<Globe size={16} />}>
                  토탈서비스
                </CarbonBtn>
                <CarbonBtn kind="secondary" size="lg" href="https://www.comwel.or.kr/comwel/intr/orgn/find.jsp" icon={<MapPin size={16} />}>
                  지사 찾기
                </CarbonBtn>
                <CarbonBtn kind="ghost" size="md" href={getFormLink()} icon={<FileDown size={16} />}>
                  공식 서식 다운
                </CarbonBtn>
              </div>

              <button onClick={() => resetCard(card.id)}
                style={{ width: '100%', padding: S.s4, marginTop: S.s4, background: 'none', border: `1px solid ${C.borderSubtle01}`, ...T.body01, color: C.textHelper, cursor: 'pointer', fontFamily: "'IBM Plex Sans', 'Noto Sans KR', sans-serif", textAlign: 'center' }}>
                체크 이력 초기화
              </button>
            </div>
          </div>
          ...
          {/* Back */}
          <div style={{ paddingTop: S.s8 }}>
            <button onClick={backToList}
              style={{ display: 'inline-flex', alignItems: 'center', gap: 6, ...T.body01, color: C.interactive, cursor: 'pointer', background: 'none', border: 'none', fontFamily: "'IBM Plex Sans', 'Noto Sans KR', sans-serif" }}>
              <ArrowLeft size={16} /> 전체 민원 목록으로
            </button>
          </div>
        </div>
      </motion.div>
  ...
  /* ── Global Shell ─────────────────────────────── */
  return (
    <div style={{ minHeight: '100vh', background: C.background, fontFamily: "'IBM Plex Sans', 'Noto Sans KR', sans-serif", color: C.textPrimary }}>

      {/* UI Shell header */}
      <header style={{ position: 'sticky', top: 0, zIndex: 50, background: C.shellBg, height: 48, display: 'flex', alignItems: 'center', padding: `0 ${S.s6}`, justifyContent: 'space-between', borderBottom: '1px solid #393939' }}>
        <button onClick={() => { setSelectedCardId(null); scrollTop(); }}
          style={{ ...T.heading01, color: C.shellText, background: 'none', border: 'none', cursor: 'pointer', fontFamily: "'IBM Plex Sans', sans-serif", letterSpacing: '.16px' }}>
          COMWEL QR
        </button>

        {/* Nav links */}
        <nav style={{ display: 'flex', alignItems: 'center', gap: 0 }}>
          {['자주 찾는 민원', '가입지원', '재활보상', '복지·지원'].map(nav => (
            <button key={nav}
              onClick={() => { setActiveCategory(nav === '복지·지원' ? '복지·지원 사업' : nav); setSelectedCardId(null); scrollTop(); }}
              style={{ height: 48, padding: `0 ${S.s5}`, background: 'none', border: 'none', ...T.body01, color: '#c6c6c6', cursor: 'pointer', fontFamily: "'IBM Plex Sans', 'Noto Sans KR', sans-serif", whiteSpace: 'nowrap', display: 'flex', alignItems: 'center' }}
        ...
        <div style={{ display: 'flex', gap: 0 }}>
          <a href="tel:1588-0075"
            style={{ height: 48, padding: `0 ${S.s5}`, display: 'flex', alignItems: 'center', ...T.body01, color: '#c6c6c6', textDecoration: 'none', fontFamily: "'IBM Plex Sans', sans-serif" }}>
            1588-0075
          </a>
          <a href="https://www.comwel.or.kr/" target="_blank" rel="noopener noreferrer"
            style={{ height: 48, padding: `0 ${S.s5}`, display: 'flex', alignItems: 'center', gap: 6, background: C.interactive, ...T.body01, color: C.textOnColor, fontWeight: 600, textDecoration: 'none', fontFamily: "'IBM Plex Sans', sans-serif" }}>
      ...
      <AnimatePresence mode="wait">
        {!activeCard ? <ListView key="list" /> : <DetailView key={`detail-${activeCard.id}`} card={activeCard} />}
      </AnimatePresence>
